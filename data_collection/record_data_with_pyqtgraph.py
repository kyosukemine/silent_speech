...
class Recorder(object):
    def __init__(self, debug=False, display=True, num_channels=None, wifi=True, EMG_num_channels=8):
        ...
        if display:
            # plot setup
            self.qtplot(EMG_num_channels)

            # thread
            # self.t_getdata = threading.Thread(target=self.getdata)
    ...
    def update(self):
        ...
        for _ in range(self.queue_audio.qsize()):
            current_audio.append(self.queue_audio.get())
            # print(current_audio[-1])
        if len(current_audio) > 0:
            self.audio_data.append(np.concatenate(current_audio, 0))
            # self.queue_audio.put(np.concatenate(current_audio, 0))
...
if __name__ == '__main__':
    with Recorder(debug=True, wifi=True, num_channels=1, display=True, EMG_num_channels=8) as r:
        pg.exec()
        # while True:
        #     # r.update()
        #     pass
